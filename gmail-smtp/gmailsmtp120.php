...
class WPGmailSMTPMailer {
    ...
    public function register_settings() {
        ...
        // We'll create sections and fields (they will appear when the tab shows the appropriate section)
        add_settings_section( 'wp_gmail_smtp_section_smtp', 'SMTP Settings', null, $this->plugin_page );
        add_settings_section( 'wp_gmail_smtp_section_sender', 'Sender Settings', null, $this->plugin_page );
        add_settings_section( 'wp_gmail_smtp_section_advanced', 'Advanced', null, $this->plugin_page );

        add_settings_field( 'gmail_address', 'Gmail Address', [ $this, 'render_text_field' ], $this->plugin_page, 'wp_gmail_smtp_section_smtp', ['id' => 'gmail_address'] );
        add_settings_field( 'gmail_password', 'App Password', [ $this, 'render_password_field' ], $this->plugin_page, 'wp_gmail_smtp_section_smtp', ['id' => 'gmail_password'] );
        add_settings_field( 'encryption', 'Encryption', [ $this, 'render_select_field' ], $this->plugin_page, 'wp_gmail_smtp_section_smtp', ['id' => 'encryption', 'options' => ['tls' => 'TLS', 'ssl' => 'SSL']] );
        add_settings_field( 'port', 'SMTP Port', [ $this, 'render_text_field' ], $this->plugin_page, 'wp_gmail_smtp_section_smtp', ['id' => 'port'] );

        add_settings_field( 'from_email', 'From Email', [ $this, 'render_text_field' ], $this->plugin_page, 'wp_gmail_smtp_section_sender', ['id' => 'from_email'] );
        add_settings_field( 'from_name', 'From Name', [ $this, 'render_text_field' ], $this->plugin_page, 'wp_gmail_smtp_section_sender', ['id' => 'from_name'] );
        add_settings_field( 'force_override', 'Force Override From', [ $this, 'render_checkbox_field' ], $this->plugin_page, 'wp_gmail_smtp_section_sender', ['id' => 'force_override', 'desc' => 'Force plugin to always override From email/name (if unchecked, only override WordPress defaults).'] );

        add_settings_field( 'debug_mode', 'Debug Mode', [ $this, 'render_checkbox_field' ], $this->plugin_page, 'wp_gmail_smtp_section_advanced', ['id' => 'debug_mode', 'desc' => 'Enable PHPMailer debug output and basic logging (only for admin use).'] );
    }
            ...
            <div style="margin-top:20px;">
                <?php if ( $active_tab === 'smtp' ) : ?>
                    <form method="post" action="options.php">
                        ...
                        // We only render the SMTP-related fields inside the sections; keep the submit button generic
                        submit_button();
                        ?>
                    </form>
                ...
                <?php elseif ( $active_tab === 'sender' ) : ?>
                    <form method="post" action="options.php">
                        <?php
                        settings_fields( 'wp_gmail_smtp_group' );
                        // Render only sender section fields by calling the settings section output
                        do_settings_sections( $this->plugin_page );
                        submit_button();
                        ?>

...
class WPGmailSMTPMailer {
    ...
    public function configure_phpmailer( $phpmailer ) {
		...
		/**------------------------------------------
		 APPLY CUSTOM “FROM” ONLY IF WP USES DEFAULTS
		 ------------------------------------------ */

		// 1. WordPress default From Email
		$default_wp_email = 'wordpress@' . preg_replace( '/^www\./', '', strtolower( $_SERVER['SERVER_NAME'] ) );

		// Apply custom FROM email only if WordPress is using its default email
		if ( strtolower( $phpmailer->From ?? '' ) === strtolower( $default_wp_email ) ) {
			if ( ! empty( $options['from_email'] ) ) {
				$phpmailer->From = $options['from_email'];
			}
		}

		// Apply custom FROM name only if WP is using its default name
		if ( strtolower( $phpmailer->FromName ?? '' ) === 'wordpress' ) {
